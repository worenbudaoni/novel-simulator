...
- maxTokens: 1024
- 降级：`llmApiKey` 为空或调用异常 → 回退 stub

**Stub 策略：** 将可用连接列表包装为选项，label 使用目标节点标题或「前往{目标节点标题}」。不足 3 个时按实际数量返回，至少保证 1 个。

**约束校验：** LLM 返回后，逐项检查 `targetNodeId` 是否在可用连接列表中。不在列表中的选项被过滤掉，并记录 warn 日志。如果过滤后不足 2 个选项，用 stub 补足。
...
| empty | 无可达连接 → 「当前没有可去的方向」提示 |
| error | 选项生成失败 → 错误提示 + 「重试」按钮 |
| LLM stub | LLM 不可用 → 降级显示连接节点标题为选项 |
| success | 正常展示 3-4 个选项 |
| 选项不足 | 过滤后 < 2 个 → stub 补足到至少 2 个 |
