...
**UI 层面**：ResolutionDisplay 覆盖 StoryViewer；事件内容硬塞 storyText；节点标题与故事区视觉断层；属性浮动动画与 resolution 生命周期耦合。
...
**删除** `page-player-story.tsx` 中事件内容拼接：
```diff
- if (res?.eventTitle) {
...
事件由 LLM 在 StoryChain 的叙事中自然融入（EventChain 生成的事件内容作为 ResolutionResult 传给 StoryChain）。
...
| `pages/page-player-story.tsx` | 修改 | 删 eventText 拼接；传 checkAttr |
...
10. 事件不硬塞 storyText，由 LLM 在故事中融入
