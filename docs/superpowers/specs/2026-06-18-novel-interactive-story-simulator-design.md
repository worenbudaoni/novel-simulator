...
- **输出**：流式故事文本（Markdown 格式，SSE 推送）
- **作用**：将用户的交互行为实时转化为故事段落

### 6.3 BranchChain（分支节点生成）
...
### 6.4 EventChain（随机事件生成）

- **触发时机**：转盘抽奖或触发随机事件，且事件池不足时
- **输入**：世界观 + 当前节点 + 小说重要节点走向 + 历史路径 + 结果类型
- **输出**：事件描述 + 属性影响
- **作用**：动态生成符合世界观的随机事件，补充预设事件池

### 6.5 运行时 LLM 切换

```java
// 用户在 Settings 配置 LLM 信息后
// 后端动态创建 OpenAiChatModel（兼容 OpenAI API 格式）
// 优先使用用户配置，否则使用 application.yml 默认配置
// 切换全局生效，无需重启
```
...
│     └─ 转盘抽奖 → 动画抽奖 → 必触发随机事件        │
│  ④ 事件引擎判断 (考虑随机率和死亡率)               │
│  ⑤ EventChain 生成事件内容（如需）                 │
│  ⑥ 更新角色属性（hp ±、攻击± 等）                │
│  ⑦ StoryChain 生成故事段落（SSE 流式输出）         │
│  ⑧ 前进到下一节点                                │
│      │                                          │
│      ├─ 到达结局节点 → 评分 → 结局页面            │
│      └─ 角色死亡 → 结局页面（游戏结束）           │
