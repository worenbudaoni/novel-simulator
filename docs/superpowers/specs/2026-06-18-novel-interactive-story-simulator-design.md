...
2. Admin 导入时 LLM 生成**核心节点 + 节点连接**（故事骨架）
3. 玩家在网状节点图中从一个核心节点开始探索
4. 到达节点时，LLM 根据**当前节点 + 可用连接 + 故事历史 + 角色属性**实时生成选项（血肉）
5. 玩家选择或转盘 → 可能触发随机事件
6. LLM 根据选择+事件实时生成故事段落（SSE 流式输出）
7. 用户到达结局节点 → 生成完整故事 + 属性评分

**核心设计理念：**
- Admin 管骨架（节点 + 连接），LLM 管血肉（选项 + 故事 + 事件）
- 选项不是固定的数据，而是每步 LLM 根据上下文动态生成的
- 这也意味着同一张节点地图，不同角色、不同选择历史，看到的选项不同
...
### 6.4 OptionChain（选项生成）

- **触发时机**：玩家到达一个节点时
- **输入**：当前节点 + 可用连接（可达节点列表）+ 故事历史 + 角色属性 + 世界观
- **输出**：3-4 个选项，每个选项包含 label + targetNodeId
- **约束**：
  - 每个选项必须指向可用连接列表中的节点，不能自由生成目标
  - 不同选项应指向不同节点，形成有意义的分支
  - 属性影响选项呈现（高智力看到更多可能性，高魅力看到社交选项）
  - LLM 不可用时从其他节点的备用选项池填充
- **运作方式**：
  ```
  玩家到达「红岸基地」
    → 查节点连接: [三体游戏, ETO集会, 叶文洁旧居]
    → OptionChain: 当前节点 + 连接列表 + 故事历史 + 角色属性
    → LLM 生成 3 个选项, 分别指向三个连接目标
    → 返回前端渲染
  ```

### 6.5 BranchChain（分支节点生成）
...
│  节点探索循环                                        │
│                                                     │
│  ① 到达节点 → OptionChain(LLM) 根据上下文生成选项    │
│     ├─ 输入：当前节点 + 连接 + 故事历史 + 角色属性    │
│     └─ 输出：3-4 个选项，各指向不同的可达节点          │
│  ② 显示当前节点描述 + LLM 生成的选项                 │
│  ③ 玩家选择                                         │
│  ④ 按概率触发转盘？                                  │
│     ├─ 是 → 转盘弹窗 → 抽奖 → EventChain(LLM)       │
│     │         生成事件+属性变化                       │
│     └─ 否 → 跳过                                    │
│  ⑤ SSE 流式：StoryChain(LLM) 根据上下文续写故事       │
│  ⑥ 更新角色属性                                     │
│  ⑦ 到达结局节点？                                    │
│     ├─ 是 → StoryChain 生成结局总结                  │
│     │      → 弹出结局弹窗（完整故事+属性回顾）        │
...
- **EventChain**：LLM 根据扇区+角色状态生成事件及属性变化
- **OptionChain**：LLM 根据节点+连接列表+故事历史+角色属性生成选项
- **属性影响选项**：LLM 自然地将属性融入选项生成（高智力看到洞察类选项，高魅力看到社交类选项），不需 `min_intelligence` 硬过滤
- **选项约束**：每个选项必须指向已连接的节点，LLM 不能自由创建
- **每次选择**：微增属性（随机 1-2 点）
